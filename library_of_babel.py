...
--book <addr> - Displays a book. Also displays the page's title.
...
def arg_check(input_array):
    ...
    try:
        for argv in input_array[1:]:
            ...
            if argv == '--book':
                coms['--book'][0] = 1
                coms['--book'][1] = input_array[input_array.index(argv) + 1]
            if argv == '--search':
                coms['--search'][0] = 1
...
def main(input_dict):
    if input_dict['--checkout'][0]:
        ...
        print(text)
        filed(input_dict, text)
    elif input_dict['--book'][0]:
        print(getTitle(input_dict['--book'][1]))
        print(getBook(input_dict['--book'][1]))
...
def getBook(address):
    ...
    for x in range(0, 410):
        result += getPage(hex_addr + ':' + wall + ':' + shelf + ':' + volume + ':' + str(x)) + '\n'
    return result
